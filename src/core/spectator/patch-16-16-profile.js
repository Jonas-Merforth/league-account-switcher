import {
  ...
  PATCH_16_16_PACKET_IDS
} from './patch-16-16-codecs.js';
import {
  levelFromExperience,
...
export function decodePatch1616Snapshot({
  blocks,
  playerBase,
  queueId = null,
  queueType = '',
  decodeHero = decodePatch1616HeroStats,
  decodeRoster = decodePatch1616RosterPayload,
  decodeTurrets = decodePatch1616TurretSnapshot
}) {
  ...
  if (!Number.isInteger(playerBase)) {
    throw new Error('The 16.16 profile requires a verified player-entity base.');
  }
  const rosterBlock = uniqueBlock(
    blocks,
    (block) => (
      block.packetId === PATCH_16_16_PACKET_IDS.roster
      && block.length >= rosterPayloadMinLength
      && block.length <= ROSTER_PAYLOAD_MAX_LENGTH
    ),
    'packet 827 roster snapshot'
  );
  ...
  if (
    ...
    throw new Error('Packet 827 did not produce ten unique participant slots.');
  }
  ...
  if (!mayhem && turretTotals === null) {
    throw new Error('Packet 320 is missing the Summoner\'s Rift turret set.');
  }
  if (!mayhem) {
  ...
  for (let slotIndex = 0; slotIndex < 10; slotIndex += 1) {
    ...
    const heroBlock = uniqueBlock(
      blocks,
      (block) => (
        block.packetId === PATCH_16_16_PACKET_IDS.heroSnapshot
        && block.param === playerParam(playerBase, slotIndex)
        && block.length === HERO_PAYLOAD_LENGTH
      ),
      `packet 248 hero snapshot for participant ${participantSlot}`
    );
    const hero = decodeHero(heroBlock.payload);
    const rosterRow = rosterBySlot.get(participantSlot);
    if (!rosterRow) {
      throw new Error(`Packet 827 is missing participant ${participantSlot}.`);
    }
    const team = teams.get(hero.teamId);
    if (!team) {
      throw new Error(`Packet 248 returned unsupported team ${hero.teamId}.`);
    }
  ...
  return {
    ...
    capabilities: {
      ...
      // Packet 793 allocates ten inventory records, but item and slot
      // semantics are not independently verified for 16.16.
      items: 'unavailable'
    }
  };
}

function matchesPatch1616Keyframe(fingerprint, blocks, queueId) {
  if (
...
export const PATCH_16_16_PROFILE = Object.freeze({
  id: 'league-16.16-scoreboard-v1',
  clientVersion: /^16\.16(?:\.|$)/,
  ...
  matchesFingerprint: (fingerprint, blocks) => (
    matchesPatch1616Keyframe(fingerprint, blocks, 420)
  ),
  decode: decodePatch1616Snapshot
});

export const PATCH_16_16_MAYHEM_PROFILE = Object.freeze({
  id: 'league-16.16-mayhem-scoreboard-v1',
  clientVersion: /^16\.16(?:\.|$)/,
  ...
  matchesFingerprint: (fingerprint, blocks) => (
    matchesPatch1616Keyframe(fingerprint, blocks, MAYHEM_QUEUE_ID)
  ),
  decode: decodePatch1616Snapshot
});
