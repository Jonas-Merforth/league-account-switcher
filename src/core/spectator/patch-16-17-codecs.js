...
const HERO_VECTOR_LENGTH = 1_492;
...
const TURRET_SNAPSHOT_PAYLOAD_MAX_LENGTH = 59;
...
// The exact-case 16.17 base champion WAD set still matches this 173-entry
// roster. Keep the alias patch-local so later patches must repeat that check.
export const PATCH_16_17_CHAMPION_IDS = PATCH_16_14_CHAMPION_IDS;
...
function decodeMutatedVarint(payload, initialCursor, transform) {
  ...
  throw new Error('16.17 packet contains an invalid mutated varint.');
}

export function decodePatch1617HeroSnapshotPayload(input) {
  const payload = Buffer.from(input);
  if (payload.length !== HERO_PAYLOAD_LENGTH || payload[0] !== 0xda) {
    throw new Error('Packet 433 does not match the verified 16.17 hero-snapshot shape.');
  }
  ...
  if (
    ...
    throw new Error('Packet 433 has an unexpected decoded vector length.');
  }
  const output = Buffer.allocUnsafe(HERO_VECTOR_LENGTH);
  for (let index = 0; index < output.length; index += 1) {
    output[index] = heroByte(payload[lengthField.cursor + index]);
  }
  return output;
}
...
function finiteFloat(vector, offset, label) {
  ...
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(`Packet 433 contains an invalid ${label}.`);
  }
  return value;
}

export function decodePatch1617HeroStats(input) {
  const vector = decodePatch1617HeroSnapshotPayload(input);
  const teamId = vector.readUInt32LE(0x20);
  if (teamId !== 100 && teamId !== 200) {
    throw new Error(`Packet 433 contains invalid team id ${teamId}.`);
  }
...
function rosterAlternatingC(value) {
  let decoded = (value - 0x15) & 0xff;
  decoded = swapAdjacentBits(decoded);
  decoded = (decoded - 0x36) & 0xff;
  return decoded ^ 0xcf;
}
...
export function decodePatch1617RosterPayload(
  input,
  championIds = PATCH_16_17_CHAMPION_IDS
) {
  const payload = Buffer.from(input);
  if (
    ...
    throw new Error('Packet 326 does not match the verified 16.17 roster shape.');
  }
  const hits = [];
  for (let offset = 0; offset < payload.length; offset += 1) {
    const matches = [];
    for (const [transform, order] of ROSTER_VARIANTS) {
      try {
        const champion = decodeMutatedString(payload, offset, transform, order);
    ...
    if (unique.length > 1) {
      throw new Error(`Packet 326 has an ambiguous champion field at byte ${offset}.`);
    }
    if (unique.length === 1) hits.push({ offset, champion: unique[0] });
  }
  if (hits.length !== 10) {
    throw new Error(`Packet 326 yielded ${hits.length} champion rows; expected ten.`);
  }
...
function turretDestroyedFromPayload(input) {
  ...
  if (
    ...
    throw new Error('Packet 786 does not match the verified 16.17 turret-snapshot shape.');
  }
  // The current generated field reader consumes bit offset 0, width 1 from
  // the retained payload header. Live early/late frames prove this is the
  // absolute destroyed flag.
  return (payload[0] & 1) === 1;
}

export function decodePatch1617TurretSnapshot(blocks) {
  if (!Array.isArray(blocks)) {
    throw new Error('The 16.17 turret decoder requires keyframe blocks.');
  }
  const expectedIds = new Set(SUMMONERS_RIFT_TURRET_OWNERS.map(([id]) => id));
  const matching = blocks.filter((block) => (
    block.packetId === PATCH_16_17_PACKET_IDS.turretSnapshot
    && expectedIds.has(block.param)
  ));
  if (matching.length === 0) return null;
  if (matching.length !== SUMMONERS_RIFT_TURRET_OWNERS.length) {
    throw new Error(`Packet 786 contains ${matching.length} Summoner's Rift turret snapshots; expected 22.`);
  }
  const byNetworkId = new Map();
  for (const block of matching) {
    if (byNetworkId.has(block.param)) {
      throw new Error(`Packet 786 contains duplicate turret network id 0x${block.param.toString(16)}.`);
    }
    if (!Buffer.isBuffer(block.payload) || block.payload.length !== block.length) {
      throw new Error('Packet 786 does not retain its complete turret payload.');
    }
  ...
  for (const [networkId, ownerTeamId] of SUMMONERS_RIFT_TURRET_OWNERS) {
    if (!byNetworkId.has(networkId)) {
      throw new Error(`Packet 786 is missing turret network id 0x${networkId.toString(16)}.`);
    }
...
export const PATCH_16_17_PACKET_IDS = Object.freeze({
  inventory: 101,
  heroSnapshot: 433,
  roster: 326,
  turretSnapshot: 786
