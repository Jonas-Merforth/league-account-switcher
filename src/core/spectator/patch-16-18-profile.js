import {
  ...
  PATCH_16_18_PACKET_IDS
} from './patch-16-18-codecs.js';
import {
  levelFromExperience,
...
export function decodePatch1618Snapshot({
  blocks,
  playerBase,
  queueId = null,
  queueType = '',
  decodeHero = decodePatch1618HeroStats,
  decodeRoster = decodePatch1618RosterPayload,
  decodeTurrets = decodePatch1618TurretSnapshot
}) {
  ...
  if (!Number.isInteger(playerBase)) {
    throw new Error('The 16.18 profile requires a verified player-entity base.');
  }
  const rosterBlock = uniqueBlock(
    blocks,
    (block) => (
      block.packetId === PATCH_16_18_PACKET_IDS.roster
      && block.length >= rosterPayloadMinLength
      && block.length <= ROSTER_PAYLOAD_MAX_LENGTH
    ),
    'packet 523 roster snapshot'
  );
  ...
  if (
    ...
    throw new Error('Packet 523 did not produce ten unique participant slots.');
  }
  ...
  if (!mayhem && turretTotals === null) {
    throw new Error('Packet 1110 is missing the Summoner\'s Rift turret set.');
  }
  if (!mayhem) {
  ...
  for (let slotIndex = 0; slotIndex < 10; slotIndex += 1) {
    ...
    const heroBlock = uniqueBlock(
      blocks,
      (block) => (
        block.packetId === PATCH_16_18_PACKET_IDS.heroSnapshot
        && block.param === playerParam(playerBase, slotIndex)
        && block.length === HERO_PAYLOAD_LENGTH
      ),
      `packet 387 hero snapshot for participant ${participantSlot}`
    );
    const hero = decodeHero(heroBlock.payload);
    const rosterRow = rosterBySlot.get(participantSlot);
    if (!rosterRow) throw new Error(`Packet 523 is missing participant ${participantSlot}.`);
    const team = teams.get(hero.teamId);
    if (!team) throw new Error(`Packet 387 returned unsupported team ${hero.teamId}.`);
  ...
  return {
    ...
    capabilities: {
      ...
      // Packet 850 allocates ten 152-byte records, but item and slot
      // semantics are not independently verified for 16.18.
      items: 'unavailable'
    }
  };
}

function matchesPatch1618Keyframe(fingerprint, blocks, queueId) {
  if (
  ...
  try {
    decodePatch1618Snapshot({ blocks, playerBase: fingerprint.playerBase, queueId });
    return true;
  } catch {
    return false;
  }
...
export const PATCH_16_18_PROFILE = Object.freeze({
  id: 'league-16.18-scoreboard-v1',
  clientVersion: /^16\.18(?:\.|$)/,
  observerDelaySeconds: 180,
  matchesContext: ({ queueId }) => SUMMONERS_RIFT_QUEUE_IDS.has(Number(queueId)),
  matchesFingerprint: (fingerprint, blocks) => matchesPatch1618Keyframe(fingerprint, blocks, 420),
  decode: decodePatch1618Snapshot
});

export const PATCH_16_18_MAYHEM_PROFILE = Object.freeze({
  id: 'league-16.18-mayhem-scoreboard-v1',
  clientVersion: /^16\.18(?:\.|$)/,
  ...
  matchesFingerprint: (fingerprint, blocks) => matchesPatch1618Keyframe(
    fingerprint,
    blocks,
    MAYHEM_QUEUE_ID
  ),
