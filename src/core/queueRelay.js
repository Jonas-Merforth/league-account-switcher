...
class RiotXmppPeerConnection {
  constructor({ credentials, log, onStanza, onClose }) {
  ...
  async connect() {
    ...
    this.live = true;
    await this.send(buildRelayPresence());
    this._drainLiveBuffer();
    return {
      boundJid: this.boundJid,
      roster: this.roster
    };
  }
  ...
  close(reason = 'stopped') {
    ...
    this.closed = true;
    this.log(`Queue relay: closing XMPP peer connection (${reason}).`);
  ...
  _drainLiveBuffer() {
    ...
    for (const stanza of extracted.stanzas) {
      Promise.resolve(this.onStanza(stanza)).catch((error) => {
        this.log(`Queue relay: stanza handler failed (${friendlyXmppError(error)}).`, 'warn');
      });
    }
  }
