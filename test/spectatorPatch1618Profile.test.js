...
import {
  decodePatch1618Snapshot,
  PATCH_16_18_MAYHEM_PROFILE,
  PATCH_16_18_PROFILE
} from '../src/core/spectator/patch-16-18-profile.js';
...
function blocks() {
  return [
    { timestamp: 1_200.5, packetId: 523, param: 0, length: 1_050, payload: Buffer.alloc(1_050) },
    ...Array.from({ length: 10 }, (_, index) => ({
      timestamp: 1_200.5,
      packetId: 387,
      param: PLAYER_BASE + index,
      length: 1_267,
      payload: Buffer.alloc(1_267, index + 1)
    })),
    ...Array.from({ length: 10 }, (_, index) => ({
      timestamp: 1_200.5,
      packetId: 850,
...
test('assembles the 16.18 scoreboard and keeps inventory unavailable', () => {
  const snapshot = decodePatch1618Snapshot({
    blocks: blocks(),
...
test('the 16.18 profiles are version, queue, and mode scoped', () => {
  assert.equal(PATCH_16_18_PROFILE.observerDelaySeconds, 180);
  assert.equal(PATCH_16_18_PROFILE.clientVersion.test('16.18.817.5716'), true);
  assert.equal(PATCH_16_18_PROFILE.clientVersion.test('16.17.810.4348'), false);
  assert.equal(PATCH_16_18_PROFILE.matchesContext({ queueId: 420 }), true);
  assert.equal(PATCH_16_18_PROFILE.matchesContext({ queueId: 400 }), true);
  assert.equal(PATCH_16_18_PROFILE.matchesContext({ queueId: 440 }), true);
  assert.equal(PATCH_16_18_PROFILE.matchesContext({ queueId: 2_400 }), false);
  assert.equal(PATCH_16_18_MAYHEM_PROFILE.matchesContext({ queueId: 2_400 }), true);
  assert.equal(PATCH_16_18_MAYHEM_PROFILE.matchesContext({ queueType: 'KIWI' }), true);
  assert.equal(PATCH_16_18_MAYHEM_PROFILE.matchesContext({ queueId: 420 }), false);
  assert.equal(PATCH_16_18_MAYHEM_PROFILE.observerDelaySeconds, 60);
});

test('rejects an incomplete 16.18 snapshot', () => {
  assert.throws(() => decodePatch1618Snapshot({
...
test('assembles Mayhem scores without Summoner\'s Rift map totals', () => {
  const snapshot = decodePatch1618Snapshot({
    blocks: blocks().filter((block) => block.packetId !== 850),
    playerBase: PLAYER_BASE,
    queueId: 2_400,
    queueType: 'KIWI',
    decodeHero: (payload) => hero(payload[0]),
...
test('accepts the shorter Mayhem roster floor but not a truncated payload', () => {
  const shortBlocks = blocks().map((block) => (
    block.packetId === 523
      ? { ...block, length: 847, payload: Buffer.alloc(847) }
      : block
  ));
  const snapshot = decodePatch1618Snapshot({
    blocks: shortBlocks,
  ...
  assert.equal(snapshot.participants.length, 10);
  assert.throws(() => decodePatch1618Snapshot({
    blocks: blocks().map((block) => (
      block.packetId === 523
        ? { ...block, length: 899, payload: Buffer.alloc(899) }
        : block
    )),
    playerBase: PLAYER_BASE,
    queueId: 420,
